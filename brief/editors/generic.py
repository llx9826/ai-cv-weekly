...
## 📌 核心判断
> 一句话概括今日{topic_name}领域最重要的变化（不超过 30 字，必须是判断）
...
## 📌 核心判断
> 一句话概括本周{topic_name}领域最重要的变化（不超过 30 字，必须是判断）
...
【字数要求】总字数 {word_lo}-{word_hi} 字，必须包含全部章节（含📌核心判断和趋势策略）。"""
...
"""
        ...
        prompt += f"""
【生成要求】:
1. 必须以📌核心判断开头，包含全部章节（含趋势策略），缺一不可
2. 总字数 {word_lo}-{word_hi} 字
3. 严格使用 ### N. 格式标记每个条目
4. 每条重点事件必须带 🦞 Claw 锐评
5. 复盘必须完整写完，不要截断
